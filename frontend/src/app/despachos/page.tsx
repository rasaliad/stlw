...
export default function DespachosPage() {
  ...
  const fetchData = async () => {
    try {
      const [userResponse, dispatchesResponse, countResponse] = await Promise.all([
        api.get<User>('/auth/me'),
        api.get<Dispatch[]>('/dispatches/'),
        api.get<{ total: number }>('/dispatches/count')
      ])
  ...
  const handleSync = async () => {
    setIsSyncing(true)
    try {
      // Hacer llamada directa ya que sap-stl no está bajo /api/v1
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const response = await api.get<Dispatch[]>(`/dispatches/?${params.toString()}`)
  ...
  const toggleRowExpanded = (dispatchId: number) => {
  ...
  const getTipoDespacho = (tipo: number): string => {
      ...
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-muted-foreground">
              Gestión de despachos del sistema STL
            </p>
          </div>
        ...
        <div className="grid gap-4 md:grid-cols-4">
          ...
          <Card className="card-cream-hover">
            ...
            <CardContent>
              <div className="text-2xl font-bold text-primary">
                {dispatches.filter(d => d.fecha_picking && 
                  new Date(d.fecha_picking).toDateString() === new Date().toDateString()
                ).length}
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="card-cream-hover">
            ...
            <CardContent>
              <div className="text-2xl font-bold text-primary">
                {new Set(dispatches.map(d => d.codigo_cliente).filter(Boolean)).size}
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="card-cream-hover">
            ...
            <CardContent>
              <div className="text-2xl font-bold text-primary">
                {dispatches.reduce((acc, d) => acc + d.lines.length, 0)}
              </div>
            </CardContent>
          </Card>
        </div>
        ...
        <Card className="card-cream shadow-cream">
          <CardHeader>
            ...
            <CardDescription>
              Mostrando {dispatches.length} de {totalCount} despachos
            </CardDescription>
          </CardHeader>
          <CardContent>
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>Fecha Picking</TableHead>
                ...
                {dispatches.map((dispatch) => (
                  <>
                    <TableRow 
                      ...
                      onClick={() => toggleRowExpanded(dispatch.id)}
                    >
                      <TableCell>
                        {expandedRows.has(dispatch.id) ? (
                      ...
                      <TableCell className="font-medium">
                        {dispatch.numero_despacho || '-'}
                      </TableCell>
                      <TableCell>
                        <div>
                          <p className="font-medium">{dispatch.nombre_cliente || '-'}</p>
                          <p className="text-sm text-muted-foreground">
                            {dispatch.codigo_cliente || '-'}
                          </p>
                        </div>
                      </TableCell>
                      <TableCell>{formatDate(dispatch.fecha_picking)}</TableCell>
                      <TableCell>{getTipoDespacho(dispatch.tipo_despacho)}</TableCell>
                      <TableCell>
                        <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${
                          dispatch.sync_status === 'SYNCED' 
                      ...
                      <TableRow>
                        <TableCell colSpan={7} className="p-0">
                          <div className="bg-muted/50 p-4">
                            <h4 className="font-semibold mb-2">Líneas del despacho</h4>
                            <Table>
                              <TableHeader>
                                <TableRow>
